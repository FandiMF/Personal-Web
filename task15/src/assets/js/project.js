...
document.addEventListener("DOMContentLoaded", () => {
    blogs = JSON.parse(localStorage.getItem("projects")) ?? [];
    renderBlog();
});
...
function addBlog(event){
    ...
    // data validate
    if (!projectName) {
        alert("Project Name not filled yet");
        return;
      } else if (!startGet) {
        alert("Start Date not filled yet");
        return;
      } else if (!endGet) {
        alert("End Date not filled yet");
        return;
      } else if (!desc) {
        alert("Description not filled yet");
        return;
      } else if (desc) {
        if (react) {
          react ? react : "";
    ...
    blogs.unshift(blog);
    console.log("Data Project :", blogs);
    localStorage.setItem("project", JSON.stringify(blogs));
    renderBlog();
